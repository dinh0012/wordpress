...
function truvatour_post_thumb( $width=300, $height='', $html=true, $crop = true) {		
  global $post;
  if( !has_post_thumbnail() ) return false;

  $fullsize = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
  $url = truvatour_image_resize( $fullsize[0], $width, $height, $crop, 'c', false );

  if($html){
    echo '<img src="'.esc_url($url).'" alt="'.esc_attr(get_the_title(get_post_thumbnail_id( $post->ID ))).'">';
  }else{
    return '<img src="'.esc_url($url).'" alt="'.esc_attr(get_the_title(get_post_thumbnail_id( $post->ID ))).'">';
  }
}
...
add_action('delete_attachment', 'truvatour_delete_resized_images');

function truvatour_image_resize($url, $width=null, $height=null, $crop=true, $align='c', $retina=false) {

  global $wpdb;

  // Get common vars (func_get_args() only get specified values)
  $common = truvatour_common_info($url, $width, $height, $crop, $align, $retina);
...
function truvatour_common_info($url, $width, $height, $crop, $align, $retina) {
...
function truvatour_delete_resized_images($post_id) {
