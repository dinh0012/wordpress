		...
		if ( $posts->have_posts() ) {
			while ( $posts->have_posts() ) :
				...
				<div id="ts-post-<?php the_ID(); ?>" class="ts-post">
					...
					<div class="ts-post-meta"><?php _e( 'Posted', 'themestall' ); ?>: <?php the_time( get_option( 'date_format' ) ); ?></div>
					<div class="ts-post-excerpt">
						<?php the_excerpt(); ?>
					</div>
					<a href="<?php comments_link(); ?>" class="ts-post-comments-link"><?php comments_number( __( '0 comments', 'themestall' ), __( '1 comment', 'themestall' ), '% comments' ); ?></a>
				</div>
