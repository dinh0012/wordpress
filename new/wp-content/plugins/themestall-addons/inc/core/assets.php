...
class Ts_Assets {
	...
	public static function register() {
		...
		wp_register_script( 'mapapi', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyCN-MbF-tNeFjUL62lar8UsgI0eE47u1Ks', array( 'jquery' ), '1.0.0-beta', true );
		...
		wp_localize_script( 'magnific-popup', 'ts_magnific_popup', array(
				'close'   => __( 'Close (Esc)', 'themestall' ),
				'loading' => __( 'Loading...', 'themestall' ),
				'prev'    => __( 'Previous (Left arrow key)', 'themestall' ),
				'next'    => __( 'Next (Right arrow key)', 'themestall' ),
				'counter' => sprintf( __( '%s of %s', 'themestall' ), '%curr%', '%total%' ),
				'error'   => sprintf( __( 'Failed to load this link. %sOpen link%s.', 'themestall' ), '<a href="%url%" target="_blank"><u>', '</u></a>' )
			) );
		// Ace
		...
		wp_localize_script( 'ts-options-page', 'ts_options_page', array(
				'upload_title'  => __( 'Choose files', 'themestall' ),
				'upload_insert' => __( 'Add selected files', 'themestall' ),
				'not_clickable' => __( 'This button is not clickable', 'themestall' )
			) );
		// Cheatsheet
		...
		wp_localize_script( 'ts-generator', 'ts_generator', array(
				'upload_title'         => __( 'Choose file', 'themestall' ),
				'upload_insert'        => __( 'Insert', 'themestall' ),
				'isp_media_title'      => __( 'Select images', 'themestall' ),
				'isp_media_insert'     => __( 'Add selected images', 'themestall' ),
				'presets_prompt_msg'   => __( 'Please enter a name for new preset', 'themestall' ),
				'presets_prompt_value' => __( 'New preset', 'themestall' ),
				'last_used'            => __( 'Last used settings', 'themestall' ),
		...
		wp_localize_script( 'ts-other-shortcodes', 'ts_other_shortcodes', array( 'no_preview' => __( 'This shortcode doesn\'t work in live preview. Please insert it into editor and preview on the site.', 'themestall' ) ) );
	...
	public static function mce_js( $plugins ) {
		$plugins['ThemeStallshortcodes'] = plugins_url( 'assets/js/tinymce.js', TS_PLUGIN_FILE );
		return $plugins;
	}
	...
	public static function skin_url( $file = '' ) {
		...
		$url = ( !$skin || $skin === 'default' ) ? plugins_url( 'assets/css/', TS_PLUGIN_FILE ) : $uploads . '/ThemeStall-skins/' . $skin;
