...
class Ts_Generator {
	...
	public static function button( $args = array() ) {
		...
		$args = wp_parse_args( $args, array(
				'target'    => $target,
				'text'      => __( 'Insert shortcode', 'themestall' ),
				'class'     => 'button',
	...
	public static function popup() {
		...
		// Cache not found
			...
			$tools = apply_filters( 'ts/generator/tools', array(
					'<a href="' . admin_url( 'admin.php?page=ThemeStall' ) . '#tab-1" target="_blank" title="' . __( 'Settings', 'themestall' ) . '">' . __( 'Plugin settings', 'themestall' ) . '</a>',
					'<a href="http://gndev.info/ThemeStall/" target="_blank" title="' . __( 'Plugin homepage', 'themestall' ) . '">' . __( 'Plugin homepage', 'themestall' ) . '</a>',
					'<a href="http://wordpress.org/support/plugin/ThemeStall/" target="_blank" title="' . __( 'Support forums', 'themestall' ) . '">' . __( 'Support forums', 'themestall' ) . '</a>'
				) );

			// Add add-ons links
			if ( !ts_addon_active( 'maker' ) || !ts_addon_active( 'skins' ) || !ts_addon_active( 'extra' ) ) $tools[] = '<a href="' . admin_url( 'admin.php?page=ThemeStall-addons' ) . '" target="_blank" title="' . __( 'Add-ons', 'themestall' ) . '" class="ts-add-ons">' . __( 'Add-ons', 'themestall' ) . '</a>';
?>
		<div id="ts-generator-wrap" style="display:none">
			<div id="ts-generator">
				<div id="ts-generator-header">
					...
					<input type="text" name="ts_generator_search" id="ts-generator-search" value="" placeholder="<?php _e( 'Search for shortcodes', 'themestall' ); ?>" />
					<p id="ts-generator-search-pro-tip"><?php printf( '<strong>%s:</strong> %s', __( 'Pro Tip', 'themestall' ), __( 'Hit enter to select highlighted shortcode, while searching' ) ) ?></p>
					<div id="ts-generator-filter">
						<strong><?php _e( 'Filter by type', 'themestall' ); ?></strong>
...
?>
	...
	public static function settings() {
		self::access();
		// Param check
		if ( empty( $_REQUEST['shortcode'] ) ) wp_die( __( 'Shortcode not specified', 'themestall' ) );
		// Get cache
		...
		// Cache not found
			...
			$actions = apply_filters( 'ts/generator/actions', array(
					'insert' => '<a href="javascript:void(0);" class="button button-primary button-large ts-generator-insert"><i class="fa fa-check"></i> ' . __( 'Insert shortcode', 'themestall' ) . '</a>',
					'preview' => '<a href="javascript:void(0);" class="button button-large ts-generator-toggle-preview"><i class="fa fa-eye"></i> ' . __( 'Live preview', 'themestall' ) . '</a>'
				) );
			// Shortcode header
			$return = '<div id="ts-generator-breadcrumbs">';
			$return .= apply_filters( 'ts/generator/breadcrumbs', '<a href="javascript:void(0);" class="ts-generator-home" title="' . __( 'Click to return to the shortcodes list', 'themestall' ) . '">' . __( 'All shortcodes', 'themestall' ) . '</a> &rarr; <span>' . $shortcode['name'] . '</span> <small class="alignright">' . $shortcode['desc'] . '</small><div class="ts-generator-clear"></div>' );
			$return .= '</div>';
			// Shortcode note
			if ( isset( $shortcode['note'] ) || isset( $shortcode['example'] ) ) {
				...
				if ( isset( $shortcode['example'] ) ) $return .= wpautop( '<a href="' . admin_url( 'admin.php?page=ThemeStall-examples&example=' . $shortcode['example'] ) . '" target="_blank">' . __( 'Examples of use', 'themestall' ) . ' &rarr;</a>' );
				$return .= '</div></div>';
			}
			// Shortcode has atts
			if ( isset( $shortcode['atts'] ) && count( $shortcode['atts'] ) ) {
				...
				foreach ( $shortcode['atts'] as $attr_name => $attr_info ) {
					...
					if ( isset( $attr_info['desc'] ) ) $return .= '<div class="ts-generator-attr-desc">' . str_replace( array( '<b%value>', '<b_>' ), '<b class="ts-generator-set-value" title="' . __( 'Click to set this value', 'themestall' ) . '">', $attr_info['desc'] ) . '</div>';
					$return .= '</div>';
				}
			}
			...
			// Wrapping shortcode
				...
				$return .= '<div class="ts-generator-attr-container"><h5>' . __( 'Content', 'themestall' ) . '</h5><textarea name="ts-generator-content" id="ts-generator-content" rows="5">' . esc_attr( str_replace( array( '%prefix_', '__' ), ts_cmpt(), $shortcode['content'] ) ) . '</textarea></div>';
			}
	...
	public static function preview() {
		...
		echo '<h5>' . __( 'Preview', 'themestall' ) . '</h5>';
	...
	public static function access() {
		if ( !self::access_check() ) wp_die( __( 'Access denied', 'themestall' ) );
	}
...
<div class="ts-generator-presets alignright" data-shortcode="<?php echo sanitize_key( $_REQUEST['shortcode'] ); ?>">
	<a href="javascript:void(0);" class="button button-large ts-gp-button"><i class="fa fa-bars"></i> <?php _e( 'Presets', 'themestall' ); ?></a>
	<div class="ts-gp-popup">
		<div class="ts-gp-head">
			<a href="javascript:void(0);" class="button button-small button-primary ts-gp-new"><?php _e( 'Save current settings as preset', 'themestall' ); ?></a>
		</div>
	...
	public static function presets_list( $shortcode = false ) {
		...
		if ( is_array( $presets ) && count( $presets ) ) {
			...
			// Hide default text
			echo sprintf( '<b style="display:none">%s</b>', __( 'Presets not found', 'themestall' ) );
		}
		// Presets doesn't found
		else echo sprintf( '<b>%s</b>', __( 'Presets not found', 'themestall' ) );
	}
...
class ThemeStall_Shortcodes_Generator extends Ts_Generator {
	function __construct() {
		parent::__construct();
	}
}
