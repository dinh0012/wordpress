...
function ts_addon_active( $addons ) {
	...
	$paths = array(
		'maker' => 'ThemeStall-maker/ThemeStall-maker.php',
		'skins' => 'ThemeStall-skins/ThemeStall-skins.php',
		'extra' => 'ThemeStall-extra/ThemeStall-extra.php',
	);
...
function ts_skins_link() {
	if ( ts_addon_active( 'skins' ) ) return sprintf( '<br><strong>%s</strong><br><strong>%s</strong>', __( 'Additional skins successfully installed', 'themestall' ), __( 'Open dropdown to choose one of new styles', 'themestall' ) );
	else return sprintf( '<br><a href="http://gndev.info/sus" target="_blank">%s &rarr;</a>', __( 'Get more styles', 'themestall' ) );
}
...
class Ts_Tools {
	...
	public static function example() {
		...
		// Cache not found
			...
			// Check for code
			if ( !$code ) die( '<p class="ts-examples-error">' . __( 'Example code does not found, please check it later', 'themestall' ) . '</p>' );
			// Clean-up the code
			...
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="ts-examples-code"><span class="ts-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'themestall' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['ts_slide_link'] = array(
			'label' => __( 'Slide link', 'themestall' ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'ts_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'ThemeStall Shortcodes', 'themestall' ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery shortcodes', 'themestall' ) )
		);
		return $form_fields;
	}
