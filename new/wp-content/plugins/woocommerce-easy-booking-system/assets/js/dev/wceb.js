	...
	$(document).ready( function() {
		...
		*/
		wceb.clearBookingSession = function() {
			...
			if ( wceb.session ) {
				...
				$.post( ajax_url.toString().replace( '%%endpoint%%', 'clear_booking_session' ), data, function( response ) {
		...
		*/
		wceb.setPrice = function() {
			...
			$.post( ajax_url.toString().replace( '%%endpoint%%', 'add_new_price' ), data, function( response ) {
				...
				if ( fragments ) {
					...
					if ( $qty_input.size() ) {
						var qty_selected = parseFloat( $qty_input.val() );
					} else {
						var qty_selected = 1;
					}
		...
		*/
		wceb.pickers.reset = function() {
			...
			if ( wceb.productType === 'variable' ) {
				...
				wceb.pickers.render( JSON.parse( ids ) );
			} else {
				wceb.pickers.render();
			}
