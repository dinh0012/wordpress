...
class WCEB_Product_Settings {
	...
	public function __construct() {
        ...
        add_action( 'woocommerce_product_addons_panel_before_options', array( $this, 'wceb_product_addons_options' ), 10, 3 );
        add_filter( 'woocommerce_product_addons_save_data', array( $this, 'wceb_product_addons_save_data' ), 10, 2 );
    ...
    * Adds option to multiply addon cost by booking duration
    *
    **/
    public function wceb_product_addons_options( $post, $addon, $loop ) {

        // Check if it is a product or a global addon
        if ( ! is_null( $post ) ) {
        ...
        <tr class="show_if_bookable">
            <td class="addon_wceb_multiply">
                

                <label for="product_addon_multiply_<?php echo $loop; ?>"><?php if ( is_null( $post ) ) {
                    esc_html_e( 'Multiply addon cost by booking duration (bookable products only)?', 'easy_booking' );
                } else {
                    esc_html_e( 'Multiply addon cost by booking duration?', 'easy_booking' );
                } ?></label>
                <input type="checkbox" name="product_addon_multiply[<?php echo $loop; ?>]" id="product_addon_multiply_<?php echo $loop; ?>" style="margin-left:4px!important;" class="checkbox" <?php checked( $multiply_addon, 1 ); ?>>
    ...
    *
    **/
    public function wceb_product_addons_save_data( $data, $i ) {
        ...
        // Also ave multiply option in each addon option to display "/ day" price.
