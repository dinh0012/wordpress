...
class WCEB_Settings {
	...
	public function __construct() {
		...
		if ( ! $this->options ) {
		    ...
		    $this->options = array(
				...
				'easy_booking_last_available_date'     => '1825',
				...
				'easy_booking_start_status'            => 'automatic',
				'easy_booking_start_status_change'     => 0,
				'easy_booking_processing_status'       => 'automatic',
				'easy_booking_completed_status'        => 'automatic',
				'easy_booking_completed_status_change' => 0
		    );
		...
		// Backward compatibility
		if ( ! isset( $this->options['easy_booking_start_status'] ) ) {
			$this->options['easy_booking_start_status'] = 'automatic';
			update_option('easy_booking_settings', $this->options);
		}

		// Backward compatibility
		if ( ! isset( $this->options['easy_booking_start_status_change'] ) ) {
			$this->options['easy_booking_start_status_change'] = 0;
			update_option('easy_booking_settings', $this->options);
		}

		// Backward compatibility
		if ( ! isset( $this->options['easy_booking_processing_status'] ) ) {
			$this->options['easy_booking_processing_status'] = 'automatic';
			update_option('easy_booking_settings', $this->options);
		}

		// Backward compatibility
		if ( ! isset( $this->options['easy_booking_completed_status'] ) ) {
			$this->options['easy_booking_completed_status'] = 'automatic';
			update_option('easy_booking_settings', $this->options);
		}

		// Backward compatibility
		if ( ! isset( $this->options['easy_booking_completed_status_change'] ) ) {
			$this->options['easy_booking_completed_status_change'] = 0;
			update_option('easy_booking_settings', $this->options);
		}

		// Backward compatibility
		if ( ! isset( $this->options['easy_booking_last_available_date'] ) ) {

			if ( isset( $this->options['easy_booking_max_year'] ) ) {
				$current_year = absint( date('Y') );
				$last_year = absint( $this->options['easy_booking_max_year'] );
				$max_year = absint( $last_year - $current_year );

				if (  $max_year <= 0 ) {
					$max_year = 1;
				}

				$last_date = $max_year * 365;
				$this->options['easy_booking_last_available_date'] = $last_date;
			} else {
				$this->options['easy_booking_last_available_date'] = '1825';
			}
		...
		if ( is_admin() ) {
			...
			add_action( 'easy_booking_settings_emails_tab', array( $this, 'easy_booking_emails_settings' ), 10);
	...
	public function wceb_settings_init() {
		...
		do_action( 'easy_booking_init_settings' );
	...
	public function wceb_emails_settings() {
		include_once( 'includes/wceb-emails-settings.php' );
	}
	...
	public function easy_booking_emails_settings() {
		echo '<p>' . __( 'These booking status are for information and organisation purposes, and are not related to WooCommerce order statuses.', 'easy_booking' ) . '</p>';

		echo '<p>' . __( 'They follow this pattern:', 'easy_booking' ) . '</p>';

		echo '<ul><li>' . __( 'Two dates: Pending > (Start) > Processing > (End) > Completed', 'easy_booking' ) . '</li><li>' . __( 'One date: Pending > Processing > Completed', 'easy_booking' ) . '</li></ul>';

		echo '<p>' . __( 'You can choose to change them manually or automatically and then easily track your bookings in the "Reports" page.', 'easy_booking' ) . '</p>';

		settings_fields( 'easy_booking_emails_settings' );
		do_settings_sections( 'easy_booking_emails_settings' );
	}
	...
	public function easy_booking_last_available_date() {
		wceb_settings_input( array(
			'type'              => 'number',
			'id'                => 'easy_booking_last_available_date',
			'name'              => 'easy_booking_settings[easy_booking_last_available_date]',
			'value'             => isset ( $this->options['easy_booking_last_available_date'] ) ? $this->options['easy_booking_last_available_date'] : '1825',
			'description'       => __('Set the last available date for all your products, relative to the current day. Max = 3650 days (10 years).', 'easy_booking'),
			'custom_attributes' => array(
				'min' => '1',
				'max' => '3650'
			)
	...
	public function easy_booking_start_section_emails() {
		echo '<p class="description">' . __( 'If you need time to prepare or deliver your items, set this status a few days before booking start date.', 'easy_booking' ) . '</p>';
	}

	public function easy_booking_start_status() {
		wceb_settings_select( array(
			'id'          => 'start_status',
			'name'        => 'easy_booking_settings[easy_booking_start_status]',
			'value'       => isset( $this->options['easy_booking_start_status'] ) ? $this->options['easy_booking_start_status'] : 'automatic',
			'options'     => array(
				'manual'    => __( 'Manually', 'easy_booking' ),
				'automatic' => __( 'Automatically', 'easy_booking' )
			)
		));

	}

	public function easy_booking_start_status_change() {
		wceb_settings_input( array(
			'type'              => 'number',
			'id'                => 'easy_booking_start_status_change',
			'name'              => 'easy_booking_settings[easy_booking_start_status_change]',
			'value'             => isset ( $this->options['easy_booking_start_status_change'] ) ? $this->options['easy_booking_start_status_change'] : 0,
			'description'       => __( 'Days before start date. Leave 0 to change on start date. Only if automatically is set.', 'easy_booking' ),
			'custom_attributes' => array(
				'min' => 0,
				'max' => 30
			)
	...
	public function easy_booking_processing_status_settings() {
		echo '<p class="description">' . __( 'If set to "Automatically", "Processing" status will start one day after booking start date.', 'easy_booking' ) . '</p>';
	}

	public function easy_booking_processing_status() {
		wceb_settings_select( array(
			'id'          => 'processing_status',
			'name'        => 'easy_booking_settings[easy_booking_processing_status]',
			'value'       => isset( $this->options['easy_booking_processing_status'] ) ? $this->options['easy_booking_processing_status'] : 'automatic',
			'options'     => array(
				'manual'    => __( 'Manually', 'easy_booking' ),
				'automatic' => __( 'Automatically', 'easy_booking' )
			)
	...
	public function easy_booking_end_status_settings() {
		echo '<p class="description">' . __( '"End" status will automatically change on booking end date. Only for two dates products.', 'easy_booking' ) . '</p>';
	}

	public function easy_booking_completed_status_settings() {
		echo '<p class="description">' . __( 'If set to "Automatically", "Completed" status will start one day after booking end date.', 'easy_booking' ) . '</p>';
	}

	public function easy_booking_completed_status() {
		wceb_settings_select( array(
			'id'          => 'completed_status',
			'name'        => 'easy_booking_settings[easy_booking_completed_status]',
			'value'       => isset( $this->options['easy_booking_completed_status'] ) ? $this->options['easy_booking_completed_status'] : 'automatic',
			'options'     => array(
				'manual'    => __( 'Manually', 'easy_booking' ),
				'automatic' => __( 'Automatically', 'easy_booking' )
			)
	...
	public function easy_booking_completed_status_change() {
		wceb_settings_input( array(
			'type'              => 'number',
			'id'                => 'easy_booking_completed_status_change',
			'name'              => 'easy_booking_settings[easy_booking_completed_status_change]',
			'value'             => isset ( $this->options['easy_booking_completed_status_change'] ) ? $this->options['easy_booking_completed_status_change'] : 0,
			'description'       => __( 'Days after end date. Leave 0 to change the day after end date. Only if automatically is set.', 'easy_booking' ),
			'custom_attributes' => array(
				'min' => 0,
				'max' => 30
			)
	...
	public function sanitize_values( $settings ) {
		...
		foreach ( $settings as $key => $value ) {
			...
			if ( $key === 'easy_booking_all_bookable' && empty( $settings['easy_booking_all_bookable'] ) ) {
